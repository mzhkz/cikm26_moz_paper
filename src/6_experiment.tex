...
このセクションの構成
あるプロンプト群があって、そこにペルソナをいれる。軸とペルソナは今回の政治ドメインでは、XXXを使います。
ペルソナをEmbedしてリクエストを飛ばして、Responseを取得。
Conjection BarrierにCitationを分解。

（モデル、対象とする政党、トピックの軸とプロンプト、プロンプトの生成は実験セクションに書く。）

最後にテクニカル貢献を述べる

This section presents citation patterns in representative and widely used GSEs within the political domains of Japan and the United States using the methodology proposed in Section~\ref{sec:methodology}.
The purpose of this experiment is twofold: (1) to reveal the potential attack surface of GSEs from the perspective of PoisonedRAG attacks in representative and widely used GSEs within the political domain, which is critical because these GSEs can significantly influence citizens' decision-making in elections, and (2) to accelerate research addressing these risks by sharing the publisher attribute perspective with the research community.
...
Our experimental goal is not to demonstrate effectiveness across exhaustive domains, but rather to illustrate the attack surface and threats that exist in the GSEs from the perspective of publisher attributes.

\input{figs/fig_tex/party_wise.tex}
...
Contemporary elections depend critically on digital information dissemination, and citizens increasingly rely on GSEs to gather information that shapes their voting decisions.
GSEs can significantly influence citizens' decision-making, rendering the threat of PoisonedRAG attacks particularly severe in this domain.
We emphasize that our experiment aims to measure actual citation patterns in the political domain rather than prescribe ideal proportions; we defer discussion of desirable citation balance for GSEs to Section~\ref{sec:discussion}.
...
Our questions are designed to be politically neutral, focusing on whether GSEs cite contents that articulate positions on the target topics rather than favoring any particular party; consequently, our findings reflect citation patterns rather than biases introduced by question design.
...
We employ three GSE models for answer generation: OpenAI GPT-5~\cite{chatgpt}, Claude Sonnet $4$ (claude-4-sonnet-20250514)~\cite{claude}, and Gemini Flash $2.0$ (gemini-2.0-flash)~\cite{gemini}, the most advanced publicly available models with web search mode enabled.
...
Note that we do not limit our evaluation criteria to closed models because our evaluation criteria do not use the internal reasoning states of GSEs but only use input and output of GSEs (question and answer).
...
\input{figs/fig_tex/citation_coverage.tex}

\subsection{Distribution of Citation Grounding Score by Content-injection Barriers}
...
% Broadly speaking, GSEs tend to reflect cited contents more accurately as the reliability of the sources is high.

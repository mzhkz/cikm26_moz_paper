...
GSEs are retrieval-and-generation systems that retrieve information from the web and generate synthesized answers with user intent and backgrounds using large language models (LLMs)~\cite{geo-apmr+24}.
% the core of 
% integrate web search results from queries transformed from user questions and generate synthesized answers while citing web content using large language models (LLMs)~\cite{geo-apmr+24}.
% GSEs receive a question from users, perform web searches, and generate answers with user intent and background using the web search results.
...
Answer generation cites the web sources obtained in the content retrieval phase and outputs the textual answer \( r \).
Web sources \( S \) are converted into a summary set \( Sum = \{Sum_1, Sum_2, \cdots, Sum_m\} \) that extracts and summarizes the content of web sources obtained in the content retrieval phase.
...
Although GSEs ideally have one or more citations for each sentence, there are cases where \( C_i = \emptyset \), meaning there are no citations for the corresponding sentence.

A core technology of GSEs is Retrieval-Augmented Generation (RAG), which retrieves content relevant to the user query from outside of LLMs and incorporates it into LLM inference~\cite{internet-augmented-language-models-fewshot-arxiv22, internet-augmented-dialogue-generation-arxiv21, rag-kdd24}.
...
Our study distinguishes GSEs and RAG systems based on the differences in external database characteristics.

...
This section introduces poisoning attacks on \emph{RAG systems} and GSEs, seminal evaluation criteria for \emph{RAG systems}, and the gap between GSEs and \emph{RAG systems} that motivates our evaluation criteria.
...
Recent studies propose methods to mitigate vulnerabilities and trustworthiness issues in citations generated by \emph{RAG systems} and GSEs.
TrustRAG~\cite{trustrag2024} presents a defense framework that removes malicious documents at the retrieval stage through clustering and matching with the internal knowledge of the LLM.
AuthGR~\cite{lee2026relevanceauthorityauthorityawaregenerative} incorporates an authority score into the retrieval process; the score is computed by a vision-language model using both textual content of web pages (e.g., article body and metadata) and visual signals from page screenshots (e.g., layout quality, design, and advertisement density).

However, these studies take the existence of citation vulnerabilities as a given premise and focus on designing defenses or improvements.
They do not systematically measure how much citation selection bias and vulnerability occur in production GSEs.
Moreover, they do not analyze the authority of citations based on the attributes of publishers, which is the focus of our work.
Our study provides a foundation that supports the necessity of such mitigation methods by revealing the scale and patterns of citation vulnerabilities in real-world GSEs.
...
Prior work has shown that LLMs produce left-leaning responses by default~\cite{santurkar2023whose, hartmann2023chatgpt}.
Tornberg et al.\ report that LLMs shift strongly to the right for conservative users but only slightly to the left for progressive users, showing an asymmetric response to personalization.

This effect extends beyond politics.
...
However, all of these prior studies focus on answers generated from the internal knowledge of LLMs, not on answers built from web search results.
No study has examined how personalization changes which sources GSEs cite or how it affects citation quality such as source authority and reflection accuracy.
...
However, since GSEs retrieve information from the open web where attackers can inject malicious content more easily than in \emph{RAG systems}, the primary concern should shift to whether GSEs retrieve even a small volume of malicious content injected by attackers during web search, which would result in a successful poisoning attack as discussed above.

To fill this gap, we propose novel evaluation criteria using publisher attributes of citations in generated answers that can assess the reliability and safety of GSEs where poisoning attacks can be a threat.
Our goal is to provide evaluation criteria for assessing the attack surface to poisoning attacks from generated answers by GSEs, not to demonstrate successful attacks themselves or to determine whether malicious content has actually been injected; rather, we aim to provide evaluation criteria for assessing the potential effectiveness of poisoning attacks based on publisher attributes of citations in observed actual answers and to reveal the attack surface of GSEs and the difficulty for poisoning attacks to succeed.

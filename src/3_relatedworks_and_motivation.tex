...
TrustRAG~\cite{trustrag2024} identifies and removes malicious documents during retrieval by clustering candidate documents and matching them with the internal knowledge of the LLM.
AuthGR~\cite{lee2026relevanceauthorityauthorityawaregenerative} calculates an authority score using a vision-language model that considers both the textual content of web pages and visual features from page screenshots (such as layout quality, design, and advertisement density).

However, these studies assumes the existence of citation vulnerabilities and focus on designing defenses or improvements.
They do not systematically measure how much citation selection bias and vulnerability occur in production GSEs.
Moreover, they do not assess citation authority by examining publisher attributes, which is a key focus of our study.
Our study establishes the need for such mitigation approaches by systematically revealing the extent and patterns of citation vulnerabilities present in real-world GSEs.
